...
//page with real timeeeeeeeeeeeee
...
const ViewUser = () => {
  const [data, setData] = useState([]);
  const socket = io(Api); // Replace with your server URL

  useEffect(() => {
      const fetchAllUsers = async () => {
          try {
              const response = await axios.get(`${Api}/Register/getUsers`);
      ...
      socket.on('message', (message) => {
          ...
          // Update data or perform actions based on real-time updates
      });
